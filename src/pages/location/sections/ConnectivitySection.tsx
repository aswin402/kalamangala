export function ConnectivitySection() {
  ...
  return (
    <section className="km-reveal relative mx-auto mt-[42px] w-full pb-[86px] pt-[42px] text-center">
      ...
      <div className="relative mx-auto w-full max-w-[1540px] px-4 sm:px-5 3xl:max-w-[1800px]">
        <h2 className="font-bold text-[34px] sm:text-[42px] md:text-[50px]">
          The Neighbourhood Advantage
        </h2>

        <div className="mx-auto mt-[34px] w-full overflow-x-auto">
          <table className="w-full table-fixed border-separate border-spacing-0 text-foreground">
            <thead>
              <tr>
                {["Landmark", "Distance", "Travel Time"].map((heading) => (
                  <th
                    key={heading}
                    className="w-1/3 border-r-[8px] border-[#fbfbef] bg-table-header px-4 py-[25px] text-center text-xl font-bold text-white last:border-r-0 md:text-2xl"
                  >
                    {heading}
                  </th>
                ))}
